...
module.exports = function(config, io) {
  ...
  const docker = require('./docker')(config, stat);
    ...
    create: {
      ...
      image: async (request, reply) => {
        ...
        if (response.statusCode == 201) {
          let image = await docker.buildImage(response.entity.toJSON().id, request.auth.credentials.id, io, Dockerfile);
          if (image.statusCode == 200) return reply(response).code(response.statusCode);
          reply(image).code(image.statusCode);
