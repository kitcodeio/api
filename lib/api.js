...
module.exports = {
  ...
  create: {
    client: async (req, res) => {
      ...
      res.status(response.status).send({token: response.entity.token});
    },
    image: async (req, res) => {
    ...
    container: async (req, res) => {
      ...
      if (image.entity.toJSON().client_id == req.user.id) {
        docker.run(image_id, ['bash'], process.stdout, {
          Tty: false
  ...
  read: async (req, res) => {
    let client_id = req.user.id;
    let model = req.params['model'];
    let response = await db.readEntry(model, 'findAll', {
      where: {
        client_id: client_id
      },
      include: {
  ...
  delete: {
    image: async (req, res) => {
      ...
      let client_id = req.user.id;
      let image = await db.readEntry(model, {
        id: id
      });
      let response;
