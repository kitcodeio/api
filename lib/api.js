  ...
  create: {
    client: async (req, res) => {
      ...
      if (subdomain.status == 200) {
        data.subdomain = subdomain.entity.toJSON()[1].salt;
        let response = await db.createEntry('Client', data);
        if (response.entity) {
          response.data = {
            token: response.entity.token
          };
          flow.create(response.entity.subdomain);
          let sub = await updateEntry('Subdomain', {
            where: {
              salt: subdomain.entity.toJSON().salt
            }
          }, {
            occupied: true
          });
    ...
    course: async (req, res) => {
      let course = req.body;
      let courseRes = await db.createEntry('Course', {
        label: course.label,
	description: course.description
      });
      if(courseRes.status == 200){
	let section = course.sections;
        let sectionRes = await db.createEntry('CourseSection', {
          course_id: courserRes.entity.id;
	  detail: course
	});
      } else {
        res.status(courseRes.status).send(courseres);
      }
    }
  },
