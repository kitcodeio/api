...
async function startContainer(container_id, salt) {
  ...
  create: {
    ...
    container: async (req, res) => {
      ...
      if (container_data.entity.length !== 0) {
        container_id = container_data.entity[0].toJSON().container_id;
        container_id = 'kide:v3';
        let container = await startContainer(container_id, salt);
        //nginx(container.ip, salt);
	//db.entry for container.id
        report = {
          status: 200,
          entity: {
