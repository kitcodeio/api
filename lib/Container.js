...
var Container = (function() {
  ...
  async function verifyImage(tutorial_id, user_id, image_id) {
    ...
    if (!image_id) { 
      image_id = (await models.Image.create(user_id, image_name)).id;
    }

    if (!await docker.inspect(image_id, 'image')) {
    ...
    let image = await docker.buildImage(image_id, user_id, modules.entity, null, tutorial_id);
    if (image.statusCode == 200) return image_id;
    return;
  }
  ...
  Container.prototype.create = async function(request, reply) {
    ...
    try {
      ...
      let image_id = await verifyImage(course_id, course_detail.submitted_by, course_detail.image_id);
