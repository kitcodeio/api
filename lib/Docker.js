...
var Docker = (function() {
  ...
  Docker.prototype.buildImage = function(id, user_id, modules, users, tutorial_id) {
    return new Promise(resolve => {
      ...
      socket.on('build:init', res => {
        ...
        } else if(!tutorial_id) return resolve({
          statusCode: 200
        });
      });
      ...
      socket.on('build:result', res => {
        ...
        if (res.statusCode == 200 && tutorial_id) {
  ...
  Docker.prototype.startContainer = async function(entry_id, container_id, salt, dependencies) {
    ...
    execSync("sudo bash " + __dirname + "/virtualhost.sh " + salt + " " + data.NetworkSettings.IPAddress);
  ...
  Docker.prototype.inspect = async function(id, type){
    let result = await request.get({
      uri: 'http://' + host + ':' + port + '/inspect/' + (type?type:'container') + '?id=' + id
    });
