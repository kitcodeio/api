...
module.exports = config => {
  ...
  async function approveTutorial(tutorial_id, label, user_id) {
    let Dockerfile = await docker.generateDockerFile(tutorial_id).catch(err => {
    ...
    if (response.statusCode == 201) {
      ...
      let image = await docker.buildImage(response.entity.toJSON().id, user_id, Dockerfile, null, tutorial_id);
