...
var Tutorial = (function() {
  ...
  Tutorial.prototype.update = async function(request, reply){
      let { id, data } = request.payload;
      data.image_id = null;
    await db.models.TutorialTags.destroy({
      where: {
        tutorial_id: id
      }
    });
    let report = await db.updateEntry('Tutorial', {
      where: { id }
    }, data);
  ...
  Tutorial.prototype.read = async function(request, reply) {
    ...
    if (request.auth.isAuthenticated) where = {};
