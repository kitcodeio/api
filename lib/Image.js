...
var Image = (function() {
  ...
  Image.prototype.create = async function(request, reply) {
    ...
    let image = await models.Image.create(client_id, label);
    
    if (!image) return reply({
      error: 'unknow error occured',
    }).code(500);
    image = await docker.buildImage(image.toJSON().id, request.auth.credentials.id, Dockerfile, io);
  ...
  Image.prototype.read = async function(request, reply) {
    ...
    let client_id = request.auth.credentials.id;
    let images = await models.Image.fetchAll(page, client_id);
